...
# (m,nx,ny)=(2,2,0): m=action number, nx=X's memory length, ny=Y's memory length
# output textfile here as dMMGA_220_test.txt
#
...
if det == 1:
    x_vc, y = np.random.random(16), np.random.random(1)
elif det == 2:
    x_vc, y = 0.6*np.ones(16), 0.6*np.ones(1)
...
def M_MATRIX(x_vc, y):
    ...
    M1_mt = x_vc*y*four_sixteen_mt
    M2_mt = x_vc*(1-y)*four_sixteen_mt
    M3_mt = (1-x_vc)*y*four_sixteen_mt
    M4_mt = (1-x_vc)*(1-y)*four_sixteen_mt
...
txt.write('#time:[Tmax,NdT] (Discretized Multi-memory Gradient Ascent) = ' + str([Tmax,NdT]) + '\n')
txt.write('#detailed of gradient: [measure=log10(dp),accuracy=log10(maxdis)] = ' + str([int(np.log10(dp)),int(np.log10(maxdis))]) + '\n')
